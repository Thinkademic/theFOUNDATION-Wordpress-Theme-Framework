...
function buildmarkup_from_query($queryargs, $optionargs, $markup = null, $return_as_array = false)
{
    ...
    if ($wp_query->have_posts()) :
        ...
        while ($wp_query->have_posts()) : $wp_query->the_post();
                                          ...
                                          if ($image != "") {
                                              $image_before = xtag("div", $image, "class=$entry_image");
                                          }
                                          if ($image_after_title) {
                                              $image_after = $image_before;
                                              $image_before = "";
                                          }
                                          if ($wrapper_class_counter) {
                                              $entry_counter = ' num_' . $counter;
                                          } // Include a class that provides unique numeric name
                                          if ($enable_unique_class_name) {
                                              $unique_class_name = $post->post_name;
                                          }
                                          if ($filtername != false) {
                                              $text = "";
                                              $filter = apply_filters($filtername, $text);
                                          } // Apply filter if specified
                                          if ($title != "") {
                                              $title = xtag("div", $title, "class=" . $entry_title_wrapper);
                                          }
